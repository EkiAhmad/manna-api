...
class Historysurprize extends Onlyus {
	...
	public function index_get($id = 0)
    {
        ...
        if ($id < 1) {
            ...
            $sql = "SELECT A.`id`,
             A.`link`,B.`name`,C.`fullname` AS `member`,CONCAT(A.`date`,' ',A.`time`) AS `date`
            FROM `history_member_surprize` A
                JOIN `merchant_store` B ON A.`id_merchant_store` = B.`id`
                JOIN `member` C ON A.`id_member` = C.`id`
